...
export function AlertsPanel() {
  ...
  const triggerTestAlert = async (id: number) => {
    try {
      await axios.post(`${API_BASE}/api/trigger`, {
        monitorId: id,
        status: 'AVAILABLE'
      });
        ...
        {monitors.map(m => (
          <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-[#1a1a1a] rounded-lg border border-[#333]">
            ...
            <button
              ...
              className="shrink-0 w-full sm:w-auto flex items-center justify-center gap-2 text-xs font-medium text-white bg-[#222] hover:bg-[#333] transition-colors py-2.5 px-4 rounded-md border border-[#444] touch-manipulation"
            >
