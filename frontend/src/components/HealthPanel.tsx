...
export function HealthPanel() {
  ...
  useEffect(() => {
    const fetchHealth = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:3001"}/api/health`);
        setHealth(res.data);
      } catch (err) {
    ...
    <div className="bg-[#111] border border-[#222] rounded-xl p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#1a1a1a] p-4 rounded-lg border border-[#333] flex items-center gap-4">
          <div className={`p-3 rounded-full ${health.status === 'healthy' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
             <Server className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs text-[#a1a1aa] font-medium tracking-wider uppercase">Backend Status</p>
            <p className="text-lg font-semibold text-white mt-1 capitalize">{health.status}</p>
          </div>
        </div>
        ...
        <div className="bg-[#1a1a1a] p-4 rounded-lg border border-[#333] flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-[#a1a1aa] font-medium tracking-wider uppercase">Uptime</p>
            <p className="text-lg font-semibold text-white mt-1">{formatUptime(health.uptime)}</p>
          </div>
        </div>
        ...
        <div className="bg-[#1a1a1a] p-4 rounded-lg border border-[#333] flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-[#a1a1aa] font-medium tracking-wider uppercase">Speicher (Heap)</p>
            <p className="text-lg font-semibold text-white mt-1">{formatBytes(health.memoryUsage.heapUsed)}</p>
          </div>
        </div>

        {/* Telegram Card */}
        <div className="bg-[#1a1a1a] p-4 rounded-lg border border-[#333] flex items-center gap-4">
          <div className={`p-3 rounded-full ${health.botConnected ? 'bg-blue-500/10 text-blue-500' : 'bg-red-500/10 text-red-500'}`}>
             <Database className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs text-[#a1a1aa] font-medium tracking-wider uppercase">Telegram Bot</p>
            <p className="text-lg font-semibold text-white mt-1">{health.botConnected ? 'Verbunden' : 'Nicht Verbunden'}</p>
          </div>
        </div>
